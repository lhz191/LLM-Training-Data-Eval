...
class BaseQualityChecker(BaseChecker):
    """
    质量评估器统一基类
    
    评估数据样本的内容质量（领域专业度、细节丰富度等）。
    不同模态、不同数据集可以有完全不同的评估方式：
    - LLM Judge（文本/多模态）
    - 规则匹配
    - 外部工具（如医学术语词典）
    
    目前主要用于: Image-to-Report (报告质量)
    """
    
    checker_type = 'quality_check'
    ...
    def check(self, data) -> Dict[str, Any]:
        ...
        Returns:
            ...
            可选字段（取决于具体实现）:
            - details: Dict, 各维度的详细说明
            - raw_response: str, LLM 原始输出（如果使用 LLM Judge）
        """
        pass
