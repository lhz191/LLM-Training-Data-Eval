...
# -*- coding: utf-8 -*-
"""
多轮 API Agent Dynamic Executability

展平所有 Session 为 APIAgentSample 列表，调用单轮 dynamic_executability。
"""
...
_st_path = os.path.join(_st_dir, 'metrics', 'dynamic_executability.py')
_spec = importlib.util.spec_from_file_location('st_dynamic_executability', _st_path)
...
st_compute_dynamic_executability = _st_mod.compute_dynamic_executability
...
def compute_dynamic_executability(
    ...
    **kwargs,
    ...
    """展平 sessions → 调用单轮 dynamic_executability。"""
    samples = _flatten(session_iterator, max_sessions)
    return st_compute_dynamic_executability(
