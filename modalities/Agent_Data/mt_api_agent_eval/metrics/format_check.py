...
# -*- coding: utf-8 -*-
"""
多轮 API Agent Format Check

展平所有 Session 为 APIAgentSample 列表，调用单轮 format_check。
"""
...
_st_path = os.path.join(_st_dir, 'metrics', 'format_check.py')
_spec = importlib.util.spec_from_file_location('st_format_check', _st_path)
...
st_compute_format_check = _st_mod.compute_format_check
...
def compute_format_check(
    ...
    **kwargs,
    ...
    """展平 sessions → 调用单轮 format_check。"""
    samples = _flatten(session_iterator, max_sessions)
    return st_compute_format_check(
