...
展平所有 Session 为 APIAgentSample 列表，调用单轮 trustworthy。
"""
...
_st_path = os.path.join(_st_dir, 'metrics', 'trustworthy.py')
_spec = importlib.util.spec_from_file_location('st_trustworthy', _st_path)
...
st_compute_trustworthy = _st_mod.compute_trustworthy
...
def compute_trustworthy(
    ...
    **kwargs,
    ...
    """展平 sessions → 调用单轮 trustworthy。"""
    samples = _flatten(session_iterator, max_sessions)
    return st_compute_trustworthy(
