...
# -*- coding: utf-8 -*-
"""
多轮 Text-GUI Agent HTML Retention

展平所有 Session 为 Record 列表，调用单轮 html_retention。
"""
...
_st_path = os.path.join(_st_dir, 'metrics', 'html_retention.py')
_spec = importlib.util.spec_from_file_location('st_html_retention', _st_path)
...
st_compute_html_retention = _st_mod.compute_html_retention
...
def compute_html_retention(
    ...
    **kwargs,
    ...
    """展平 sessions → 调用单轮 html_retention。"""
    records = _flatten(session_iterator, max_sessions)
    return st_compute_html_retention(
