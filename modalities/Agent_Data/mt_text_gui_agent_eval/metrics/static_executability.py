...
# -*- coding: utf-8 -*-
"""
多轮 Text-GUI Agent Static Executability

展平所有 Session 为 Record 列表，调用单轮 static_executability。
"""
...
_st_path = os.path.join(_st_dir, 'metrics', 'static_executability.py')
_spec = importlib.util.spec_from_file_location('st_static_executability', _st_path)
...
st_compute_static_executability = _st_mod.compute_static_executability
...
def compute_static_executability(
    ...
    **kwargs,
    ...
    """展平 sessions → 调用单轮 static_executability。"""
    records = _flatten(session_iterator, max_sessions)
    return st_compute_static_executability(
