...
def compute_static_executability(
    ...
    total_actions = 0
    total_errors = 0      # Action 级别的总错误数
    total_warnings = 0    # Action 级别的总警告数
...
def _save_summary_log(results: Dict[str, Any], summary_file: str, elapsed: float, dataset_name: str):
    ...
    lines.append(f"  总警告数: {results.get('total_warnings', 0):,}")
    lines.append("")
    ...
    if dataset_stats:
        ...
        # WebLINX 特殊格式
        if 'uid_required_count' in dataset_stats:
            uid_required = dataset_stats.get('uid_required_count', 0)
            page_found = dataset_stats.get('page_found_count', 0)
            uid_success = dataset_stats.get('uid_success_count', 0)
            coord_success = dataset_stats.get('coord_success_count', 0)
            attr_success = dataset_stats.get('attr_success_count', 0)
            
            denom = page_found if page_found > 0 else 1
            
            lines.append(f"  uid_required: {uid_required} (需要定位的 action)")
            lines.append(f"  uid_success: {uid_success:,}/{page_found:,} ({uid_success/denom:.1%})")
            lines.append(f"  coord_success: {coord_success:,}/{page_found:,} ({coord_success/denom:.1%})")
            lines.append(f"  attr_success: {attr_success:,}/{page_found:,} ({attr_success/denom:.1%})")
        
        # Mind2Web 格式
        elif 'coord_success' in dataset_stats or 'attr_success' in dataset_stats:
            verified = dataset_stats.get('verified_actions', results['total_actions'])
            denom = verified if verified > 0 else 1
            
            if 'coord_success' in dataset_stats:
                coord = dataset_stats['coord_success']
                lines.append(f"  coord_success: {coord:,}/{verified:,} ({coord/denom:.1%})")
            if 'attr_success' in dataset_stats:
                attr = dataset_stats['attr_success']
                lines.append(f"  attr_success: {attr:,}/{verified:,} ({attr/denom:.1%})")
    ...
    if 'uid_required_count' in dataset_stats:
        ...
        coord_success = dataset_stats.get('coord_success_count', 0)
        attr_success = dataset_stats.get('attr_success_count', 0)
        lines.append(f"  🆔 UID 定位成功率: {uid_success/denom:.2%}")
        lines.append(f"  📍 坐标定位成功率: {coord_success/denom:.2%}")
        lines.append(f"  🏷️ 属性定位成功率: {attr_success/denom:.2%}")
...
def _print_summary(results: Dict[str, Any], elapsed: float, dataset_name: str):
    ...
    print(f"  总警告数: {results.get('total_warnings', 0):,}")
    print()
    ...
    if dataset_stats:
        ...
        # WebLINX 特殊格式：显示 success/total (rate%)
        if 'uid_required_count' in dataset_stats:
            uid_required = dataset_stats.get('uid_required_count', 0)
            page_found = dataset_stats.get('page_found_count', 0)
            uid_success = dataset_stats.get('uid_success_count', 0)
            ...
            # 使用 page_found 作为分母（因为 page_not_found 无法定位）
            denom = page_found if page_found > 0 else 1
            
            print(f"  uid_required: {uid_required} (需要定位的 action)")
            print(f"  uid_success: {uid_success:,}/{page_found:,} ({uid_success/denom:.1%})")
            print(f"  coord_success: {coord_success:,}/{page_found:,} ({coord_success/denom:.1%})")
            print(f"  attr_success: {attr_success:,}/{page_found:,} ({attr_success/denom:.1%})")
        ...
        # Mind2Web 格式
            ...
            if 'coord_success' in dataset_stats:
                coord = dataset_stats['coord_success']
                print(f"  coord_success: {coord:,}/{verified:,} ({coord/denom:.1%})")
            if 'attr_success' in dataset_stats:
                attr = dataset_stats['attr_success']
                print(f"  attr_success: {attr:,}/{verified:,} ({attr/denom:.1%})")
