...
def run_static_executability(
    ...
    if dataset_key == 'mind2web':
        ...
        # 加载数据 - 先完整解析所有数据
        loader = WebLINXLoader(config['data_path'], split='train')
        ...
        # raw_data 路径（包含 demonstrations/）
        raw_data_path = config.get('raw_data_path', '/home/liuhaoze/Downloads/raw_data')
        
        # 创建检查器
        checker = WebLINXStaticChecker(
            raw_data_path=raw_data_path,
            headless=not show_browser,
        )
    
    # 运行评估
    # mind2web, webshop, weblinx 都使用预先解析好的 all_records
    if dataset_key in ('mind2web', 'webshop', 'weblinx'):
...
def run_dynamic_executability(
    ...
    if dataset_key == 'mind2web':
        from loaders import Mind2WebLoader
        from executor.mind2web import Mind2WebDynamicChecker
...
def run_format_check(
    ...
    if dataset_key == 'mind2web':
        from loaders import Mind2WebLoader
        from executor.mind2web import Mind2WebFormatChecker
        ...
        from loaders import WebShopLoader
        from executor.webshop import WebShopFormatChecker
        ...
        from loaders import WebLINXLoader
        from executor.weblinx import WebLINXFormatChecker
