...
class AnswerExtractor(BaseAnswerExtractor):
    """
    答案提取器基类（Math）
    
    继承自 common.base.BaseAnswerExtractor
...
class CodeExtractor(BaseCodeExtractor):
    """
    代码提取器基类（Math）
    
    继承自 common.base.BaseCodeExtractor
...
class CodeExecutor(BaseCodeExecutor):
    ...
    继承自 common.base.BaseCodeExecutor
...
class ResultComparator(BaseResultComparator):
    """
    结果比较器基类（Math）
    
    继承自 common.base.BaseResultComparator
...
def get_comparator(dataset_type: str) -> ResultComparator:
    ...
    if 'lila' in dataset_type:
        from executor.lila import LILAResultComparator
        return LILAResultComparator()
    elif 'openmath' in dataset_type or 'math' in dataset_type:
        from executor.openmath import OpenMathResultComparator
