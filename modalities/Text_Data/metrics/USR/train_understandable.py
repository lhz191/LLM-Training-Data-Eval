...
def predict(args, model, tokenizer, data_dir, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        eval_dataset = load_pred_examples(args, data_dir, eval_task, tokenizer, evaluate=True)
        ...
        if args.output_mode == "classification":
            preds = torch.softmax(torch.Tensor(preds.tolist()), dim=-1)[:,1]
                  
        elif args.output_mode == "regression":
            preds = np.squeeze(preds)

    #print("skipping writing")
    open(data_dir + 'dr.scores', 'w+').write(str(preds.tolist()))
    return preds
...
def load_pred_examples(args, data_dir, task, tokenizer, evaluate=True):
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            predict(args, model, tokenizer, args.data_dir, prefix=prefix)
            quit()
