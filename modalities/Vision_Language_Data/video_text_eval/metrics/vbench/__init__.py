...
class VBench(object):
    ...
    def build_full_info_json(self, videos_data, name, dimension_list, prompt_list=[], special_str='', verbose=False, mode='vbench_standard', **kwargs):
        ...
        if mode=='custom_input':
            ...
            cur_full_info_list = []
            video_names = [v['video'] for v in videos_data]
    ...
    def evaluate(self, video_data, name, prompt_list=[], dimension_list=None, local=False, read_frame=False, mode='vbench_standard', **kwargs):
        results_dict = {}
        if dimension_list is None:
            dimension_list = self.build_full_dimension_list()
        submodules_dict = init_submodules(dimension_list, local=local, read_frame=read_frame)

        cur_full_info_path = self.build_full_info_json(video_data, name, dimension_list, prompt_list, mode=mode, **kwargs)
        
        for dimension in dimension_list:
            try:
                dimension_module = importlib.import_module(f'.{dimension}', package='metrics.vbench')
            ...
            results, _ = evaluate_func(cur_full_info_path, self.device, submodules_list, **kwargs)
